...
def _require_field(value: dict[str, Any], field: str, path: str) -> None:
    if field not in value:
        raise P6ContractError(f"{path} missing required field: {field}")


def _require_fields(value: dict[str, Any], fields: tuple[str, ...], path: str) -> None:
    missing = [field for field in fields if field not in value]
    if missing:
        raise P6ContractError(f"{path} missing required fields: {', '.join(missing)}")


def _require_dict(value: Any, path: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise P6ContractError(f"{path} must be an object")
    return value


def _require_list(value: Any, path: str, *, min_items: int = 0) -> list[Any]:
    if not isinstance(value, list):
        raise P6ContractError(f"{path} must be a list")
    if len(value) < min_items:
        raise P6ContractError(f"{path} must contain at least {min_items} item(s)")
    return value


def _require_non_empty_text(value: Any, path: str) -> str:
    if value is None:
        raise P6ContractError(f"{path} must not be empty")
