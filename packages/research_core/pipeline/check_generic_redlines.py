...
BRAND_SELLER_KEYS = {
    "brand", "brand_name", "seller", "seller_name", "supplier",
    "supplier_name", "store", "store_name", "storename",
}
CATEGORY_KEYWORDS_KEYS = {
    "category_name", "category_ref", "category_path", "primary_category",
    "subcategory", "subcategory_name", "keyword", "keywords", "top_keyword",
    "main_keyword", "search_name", "searchname", "product_name", "productname", "market_label",
}
ROUTE_PRODUCT_TERM_KEYS = {
    "candidate_name", "profile_id", "route_name", "route_label",
    "task_name", "target_product", "target_route",
}
TOKEN_SKIP_VALUES = {
    "us", "usa", "amazon", "amz", "fba", "fbm", "rmb", "cny", "usd",
    "true", "false", "none", "null", "strong", "medium", "weak", "high", "low",
    "main_check", "supplement_check", "exclude",
    "primary_reference", "high_sales_benchmark", "new_release_sample",
    "premium_benchmark", "painpoint_reference", "excluded_reference",
    "base_core", "upgraded_core", "scenario_specialized", "bundle_or_set",
    "feature_material_upgrade", "adjacent_or_watch",
    "primary_market", "primary_market_distribution_grouped",
    "broad_market", "subcategory_market", "mixed_pool",
    "watch", "keep", "primary", "secondary",
    "category_report", "category_report_from_history", "category_trend",
    "product_traffic_terms", "competitor_product_keywords",
    "keyword_extends", "keyword_detail", "similar_product_feature",
    "potential_product", "search_categories_broadly", "category_search_from_product_name",
}
RUN_TOKEN_FILE_SUFFIXES = {".json", ".md", ".txt"}
RUN_TOKEN_EXCLUDE_DIRS = {"inputs", "seller_sprite", "reviews", "review", "1688", "exports", "raw", "attachments"}
...
def add_terms_from_value(
    groups: dict[str, set[str]], group: str, value: object, allow_short: bool = False,
) -> None:
...
def collect_run_redline_terms(run_dir: Path | str) -> dict[str, tuple[str, ...]]:
    ...
    limits = {
        "run_asins": 250, "run_node_ids": 100, "run_keywords_categories": 200,
        "run_route_or_product_terms": 120, "run_brands_sellers": 100, "run_identity": 20,
    }
    return {
