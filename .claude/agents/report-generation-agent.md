...
description: "Stage 12 报告生成，基于 seed 和最终判断写 report_data.json 与正式 HTML 报告。"
---
# Report Generation Agent

Generated by scripts/generate_agent_adapters.py. Do not edit manually.

source_file: `skills/amazon-product-research/agents/report-generation-agent.md`
source_sha256: `ecc17f0d59d04c6bd4cad0df1e2af8201e5e439a1d8784b05a131f6d8a446ab6`
routing_hint: `Stage 12 报告生成，基于 seed 和最终判断写 report_data.json 与正式 HTML 报告。`
routing_policy: `routing_hint is only for runtime discovery; the canonical instructions below are authoritative.`
stage: `stage_12_report_generation`
category: `report`
fallback_policy: `independent_agent_recommended`
max_retries: `3`

## Runtime Contract

始终使用简体中文。你是 `Report Generation Agent`，只处理本 Agent 的职责范围。
...
<section class="section">
  ...
  <table class="go-nogo" style="margin-top:20px;">
    ...
    <tbody>
      <tr>
        <td class=”tc”>条件名</td><td>放行标准</td><td>暂停标准</td>
        <td class=”tc”><span class=”tag tag-gray”>待验证</span></td>
      </tr>
    </tbody>
  </table>
  <p class=”table-note”>放行条件表“决策条件”和“当前状态”是短标签/短值列，必须加 <code>class=”tc”</code> 居中。“放行条件”和“暂停条件”是长文本列，左对齐保持可读性。当前状态用 <code>&lt;span class=”tag tag-gray”&gt;待验证&lt;/span&gt;</code> 或 <code>tag-green</code> / <code>tag-red</code> 对应已满足/不满足。</p>
</section>
</div>
</body>
```
...
- **品牌集中度列**：只列有份额%的品牌，格式 `BrandA (15.5%) · BrandB (14.7%) · BrandC (14.4%) · BrandD (7.2%)`。数据来自 `category_panorama.categories[*].brand_concentration.brands`。禁止加”等 N 品牌”后缀，禁止堆砌无份额%的品牌名。如果该类目无品牌集中度数据，写 `—（严重混池，不适用）`。
