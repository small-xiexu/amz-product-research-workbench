...
- **所有模式都必须输出产品路线矩阵**：把已发现的款式拆成主线、升级、旁支、功能升级、观察/排除，并写明每条路线当前数据证据、代表 ASIN/供应商、风险和下一步补数动作。
...
- **主线产品形态**：什么样的产品算目标（如"带腰包/腰带的免手持牵引绳"）
- **升级路线**：有明显客单价或场景差异的款式（如"双牵引绳 + 腰包/腰带"）
- **旁支观察**：相近但不完全符合主线的款式（如"一拖二/双头多狗绳但没有腰包"）
...
硬性要求：
- 主线和升级路线只要有市场 ASIN 或 1688 候选，就必须做路线级小深挖。
- 旁支路线可以轻量验证，但必须说明为什么不进主推。
- 功能升级（反光、防爆冲、双手柄等）通常作为规格维度验证，不默认单独立项。
