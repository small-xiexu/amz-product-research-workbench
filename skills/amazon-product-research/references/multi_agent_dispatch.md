...
| Stage 7 报告生成 | 不 spawn | 证据包齐全 | AI 主 Agent 直接手写 HTML + 脚本跑 XLSX | `<中文品名>_分析报告.html`、`<中文品名>_数据回表.xlsx` |
...
3. AI 主 Agent：读全部 Evidence Packet + route_matrix，以资深运营专家身份先写 `report_data.json`，再手写 `<中文品名>_分析报告.html`。
4. 脚本：运行 `python3 -m packages.research_core.pipeline.build_analysis_report <run_dir>` 生成 XLSX + QA（脚本不生成 HTML）。
5. Delivery QA Agent：检查文件、证据、越权和通用模板风险。
