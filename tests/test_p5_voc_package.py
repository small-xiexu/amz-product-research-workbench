"""P5-2 tests: review_voc_package generation from review plugin exports."""
...
class VocPackageHappyPathTests(unittest.TestCase):
    """Happy-path tests for VOC package generation."""
    ...
    def _write_review_xlsx(self, reviews: list[dict] | None = None) -> Path:
        if reviews is None:
            reviews = _sample_reviews()
        path = self._tmp / "test_reviews.xlsx"
        _write_review_xlsx_file(path, reviews)
        return path

    def _write_review_json(self, reviews: list[dict] | None = None) -> Path:
        if reviews is None:
            reviews = _sample_reviews()
        path = self._tmp / "test_reviews.json"
        path.write_text(json.dumps({"reviews": reviews}, ensure_ascii=False, indent=2), encoding="utf-8")
        return path

    def test_xlsx_happy_path(self) -> None:
        """Generate review_voc_package from a valid xlsx export."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._write_review_xlsx()

        outputs = run_review_voc_package(run_dir, xlsx_path)
        package_path = outputs["voc_package"]
        self.assertTrue(package_path.exists())
        package = load_json(package_path)

        validate_review_voc_package_p5(package)
        self.assertEqual(package["schema_version"], "p5-voc-gate-v1")
        self.assertEqual(package["metadata"]["run_id"], "20260624_generic_direction")
        self.assertIn("review_voc/review_asin_batch.json", package["metadata"]["source_batch_ref"])
        self.assertGreater(package["stats"]["review_count"], 0)
        self.assertGreater(len(package["normalized_reviews"]), 0)
        self.assertEqual(package["pain_points"], [])
        self.assertEqual(package["highlights"], [])
        self.assertEqual(package["opportunity_hypotheses"], [])

        # Every review has required traceability fields
        for review in package["normalized_reviews"]:
            self.assertIn("review_id", review)
            self.assertIn("asin", review)
            self.assertIn("rating", review)
            self.assertIn("source_file", review)

    def test_progress_updated_after_package_generation(self) -> None:
        """Progress is correctly updated after P5-2 runs."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._write_review_xlsx()
        ...
        self.assertIn("review_voc/review_voc_package.json", progress["completed_artifacts"])
        next_action = progress.get("next_action", {})
        self.assertEqual(next_action.get("next_substage"), "P5-3")

    def test_json_input_mode(self) -> None:
        """JSON workbench input produces valid package."""
        run_dir = self._seed_p5_1_done()
        json_path = self._write_review_json()

        outputs = run_review_voc_package(run_dir, json_input=json_path)
        package = load_json(outputs["voc_package"])
        validate_review_voc_package_p5(package)
        self.assertGreater(package["stats"]["review_count"], 0)

    def test_html_auxiliary_input(self) -> None:
        """HTML AI report is included as auxiliary reference."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._write_review_xlsx()
        html_path = self._tmp / "ai_report.html"
        html_path.write_text("<html><div class='markdown-body'><p>summary text</p></div></html>", encoding="utf-8")

        outputs = run_review_voc_package(run_dir, xlsx_path, html_path)
        package = load_json(outputs["voc_package"])
        validate_review_voc_package_p5(package)
        ref = package.get("ai_report_reference", {})
        self.assertIn("test_reviews.xlsx", package["metadata"].get("source_files", [])[0])
        self.assertTrue(
            any("ai_report.html" in str(f) for f in ref.get("source_files", [])),
            "HTML should be noted in ai_report_reference"
        )

    def test_data_gaps_populated_for_low_review_count(self) -> None:
        """When review count < 30, data_gaps includes low_review_count."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._write_review_xlsx(_sample_reviews(count=5))

        run_review_voc_package(run_dir, xlsx_path)
        package = load_json(run_dir / "review_voc" / "review_voc_package.json")
        gaps = package.get("data_gaps", [])
        self.assertTrue(
            any(g.get("gap_type") == "low_review_count" for g in gaps),
            "should flag low review count"
        )

    def test_compatible_with_legacy_validator(self) -> None:
        """P5 package passes the legacy validate_review_voc_package."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._write_review_xlsx()
        run_review_voc_package(run_dir, xlsx_path)
        package = load_json(run_dir / "review_voc" / "review_voc_package.json")
        # Legacy validator should not raise
        validate_review_voc_package(package)


class VocPackageErrorTests(unittest.TestCase):
    ...
    def test_no_input_files_raises(self) -> None:
        """No input files or --json-input raises P5VocPackageError."""
        run_dir = self._seed_p5_1_done()
        with self.assertRaises(P5VocPackageError):
            run_review_voc_package(run_dir)

    def test_missing_xlsx_file_logs_gap_and_still_produces_package(self) -> None:
        """Missing xlsx produces package with file_missing and no_reviews gaps."""
        run_dir = self._seed_p5_1_done()
        nonexistent = self._tmp / "does_not_exist.xlsx"
        run_review_voc_package(run_dir, nonexistent)
        package = load_json(run_dir / "review_voc" / "review_voc_package.json")
        gaps = package.get("data_gaps", [])
        gap_types = {g["gap_type"] for g in gaps}
        self.assertIn("file_missing", gap_types)
        self.assertIn("no_reviews", gap_types)

    def test_empty_xlsx_still_produces_package(self) -> None:
        """Empty xlsx produces minimal package with data_gaps."""
        run_dir = self._seed_p5_1_done()
        path = self._tmp / "empty.xlsx"
        _write_review_xlsx_file(path, [])  # empty sheet
        # openpyxl will create an empty sheet with no rows, which should return []
        # But we need at least a header row for the parser to work
        # Just test with a file that has no reviews
        run_review_voc_package(run_dir, path)
        package = load_json(run_dir / "review_voc" / "review_voc_package.json")
        gaps = package.get("data_gaps", [])
        self.assertTrue(any(g["gap_type"] in ("no_reviews", "low_review_count") for g in gaps))

    def test_missing_asin_batch_blocks_voc_package(self) -> None:
        """Missing asin_batch is required — P5-2 depends on P5-1 batch."""
        run_dir = self._seed_p5_1_done()
        batch_path = run_dir / "review_voc" / "review_asin_batch.json"
        batch_path.unlink()
        xlsx_path = self._tmp / "test_reviews.xlsx"
        _write_review_xlsx_file(xlsx_path, _sample_reviews(10))

        with self.assertRaises(P5VocPackageError):
            run_review_voc_package(run_dir, xlsx_path)

    def test_bad_format_xlsx_produces_gap(self) -> None:
        """Corrupted xlsx is caught and reported as gap."""
        run_dir = self._seed_p5_1_done()
        bad_path = self._tmp / "bad.xlsx"
        bad_path.write_text("not an excel file", encoding="utf-8")

        run_review_voc_package(run_dir, bad_path)
        package = load_json(run_dir / "review_voc" / "review_voc_package.json")
        gaps = package.get("data_gaps", [])
        gap_types = {g["gap_type"] for g in gaps}
        self.assertIn("parse_error", gap_types)


class VocPackageScopeTests(unittest.TestCase):
    """Scope verification: P5-2 does not produce downstream artifacts."""
    ...
    def test_no_voc_gate_generated(self) -> None:
        """P5-2 does not generate voc_gate.json or voc_evidence_packet.json."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._tmp / "test_reviews.xlsx"
        _write_review_xlsx_file(xlsx_path, _sample_reviews(10))
        run_review_voc_package(run_dir, xlsx_path)

        self.assertFalse((run_dir / "review_voc" / "voc_gate.json").exists())
        self.assertFalse((run_dir / "review_voc" / "voc_evidence_packet.json").exists())

    def test_no_report_artifacts_generated(self) -> None:
        """P5-2 does not produce HTML, XLSX, or formal report artifacts."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._tmp / "test_reviews.xlsx"
        _write_review_xlsx_file(xlsx_path, _sample_reviews(10))
        run_review_voc_package(run_dir, xlsx_path)
        ...
        self.assertEqual(len(html_files), 0, "no HTML should be generated in P5-2")
        self.assertEqual(len(xlsx_files), 0, "no XLSX should be generated in P5-2")

    def test_no_voc_report_md_generated(self) -> None:
        """P5-2 does not generate voc_report.md or voc_summary.md (P5 only)."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._tmp / "test_reviews.xlsx"
        _write_review_xlsx_file(xlsx_path, _sample_reviews(10))
        run_review_voc_package(run_dir, xlsx_path)

        self.assertFalse((run_dir / "review_voc" / "voc_report.md").exists())
        self.assertFalse((run_dir / "review_voc" / "voc_summary.md").exists())
        self.assertFalse((run_dir / "review_voc" / "voc_evidence.xlsx").exists())


class VocPackageContractTests(unittest.TestCase):
    """Contract validator tests for review_voc_package_p5."""

    def test_validator_accepts_minimal_package(self) -> None:
        validate_review_voc_package_p5(_minimal_package())
    ...
    def test_validator_rejects_missing_metadata(self) -> None:
        ...
        del pkg["metadata"]
        with self.assertRaises(P5ContractError):
            validate_review_voc_package_p5(pkg)

    def test_validator_rejects_missing_run_id(self) -> None:
        pkg = _minimal_package()
        del pkg["metadata"]["run_id"]
        with self.assertRaises(P5ContractError):
            validate_review_voc_package_p5(pkg)

    def test_validator_rejects_missing_stats(self) -> None:
        pkg = _minimal_package()
        del pkg["stats"]
        with self.assertRaises(P5ContractError):
            validate_review_voc_package_p5(pkg)

    def test_validator_rejects_empty_reviews_without_id_or_text(self) -> None:
        pkg = _minimal_package()
        pkg["normalized_reviews"] = [{"review_id": "", "review_text": "", "review_text_zh": ""}]
        with self.assertRaises(P5ContractError):
            validate_review_voc_package_p5(pkg)


class VocPackageCliSmokeTests(unittest.TestCase):
    """CLI smoke tests for the P5-2 entry point."""
    ...
    def _run_cli(self, *args: str) -> subprocess.CompletedProcess[str]:
        script = ROOT / "scripts" / "build_review_voc_package.py"
        cmd = [sys.executable, str(script)] + list(args)
        return subprocess.run(cmd, capture_output=True, text=True, timeout=60)

    def test_cli_xlsx_smoke(self) -> None:
        """CLI with xlsx exits 0 and writes package."""
        run_dir = self._seed_p5_1_done()
        xlsx_path = self._tmp / "test_reviews.xlsx"
        _write_review_xlsx_file(xlsx_path, _sample_reviews(10))

        result = self._run_cli(str(run_dir), str(xlsx_path))
        self.assertEqual(result.returncode, 0, f"CLI failed: {result.stderr}")
        self.assertTrue((run_dir / "review_voc" / "review_voc_package.json").exists())

    def test_cli_json_input_smoke(self) -> None:
        """CLI with --json-input exits 0."""
        run_dir = self._seed_p5_1_done()
        json_path = self._tmp / "test_reviews.json"
        json_path.write_text(json.dumps({"reviews": _sample_reviews(10)}, ensure_ascii=False), encoding="utf-8")

        result = self._run_cli(str(run_dir), "--json-input", str(json_path))
        self.assertEqual(result.returncode, 0, f"CLI failed: {result.stderr}")
        self.assertTrue((run_dir / "review_voc" / "review_voc_package.json").exists())
...
def _sample_reviews(count: int = 35) -> list[dict[str, Any]]:
    ...
    for i in range(1, count + 1):
        ...
        reviews.append({
            ...
            "body_en": f"This is the review body for review {i}. The product works well for most use cases.{' However, there is a durability concern after extended use.' if rating <= 2 else ''}",
            "title_zh": f"评论标题{i}",
            "body_zh": f"这是第{i}条评论的正文。产品在大多数场景下表现良好。{'但长期使用后存在耐用性问题。' if rating <= 2 else ''}",
...
def _minimal_package() -> dict[str, Any]:
    return {
        "schema_version": "p5-voc-gate-v1",
        "metadata": {
            "package_id": "review-voc-test",
            "generated_at": "2026-06-24T00:00:00Z",
            "source_type": "review_plugin_export",
            "run_id": "test_run",
            "source_files": ["test.xlsx"],
            "source_batch_ref": "review_voc/review_asin_batch.json",
            "integration_note": "Excel is the structured source of truth.",
            "analysis_note": "痛点分析由 VOC Evidence Agent 完成。",
        },
        "stats": {
            "review_count": 35,
            "asin_count": 8,
            ...
            "rating_distribution": {"5星": 10},
            "sentiment_distribution": {"正面": 20},
            "low_rating_count": 10,
            "verified_count": 25,
            "media_review_count": 5,
            "entry_site_distribution": [{"name": "US", "count": 35}],
            "review_region_distribution": [{"name": "US", "count": 35}],
            "primary_entry_site": "US",
            "primary_review_region": "US",
            "source_scope_note": "站点不等同于目标市场。",
        },
        "ai_report_reference": {"source_files": [], "text_excerpt": "", "usage": "辅助阅读"},
        "normalized_reviews": [
            {
                "review_id": "R000001",
                "asin": "B0XXXXX001",
                ...
                "review_text_zh": "好产品。",
                "url": "https://amazon.com/review/R000001",
            ...
            },
            {
                ...
                "verified": "是",
                "helpful_count": 5,
                "review_text": "Broke after two weeks. Very disappointed.",
                "review_text_zh": "两周就坏了，非常失望。",
                "url": "https://amazon.com/review/R000002",
                "source_file": "test.xlsx",
            },
        ],
        "summary": {},
