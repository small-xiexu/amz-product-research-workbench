"""P6 regression tests: E2E chain, progress flow, CLI smoke, scope boundaries.

These tests complement test_p6_contracts.py (35 contract-level tests).
They verify cross-stage integration: seed P4→P5→P6 and validate all 7 outputs.
"""
...
class P6EndToEndTests(unittest.TestCase):
    """Seed P4 → P5-1 → P5-2 → P5-3 → P6, verify all 7 outputs."""
    ...
    def _seed_p5_3_done(self, review_count: int = 35) -> Path:
    ...
    def test_e2e_all_seven_outputs_delivered(self) -> None:
        """P5 → P6 produces all 7 evaluation files."""
        run_dir = self._seed_p5_3_done(35)
        outputs = run_evaluations(run_dir)

        eval_dir = run_dir / "evaluations"
        self.assertTrue(eval_dir.exists())
        for artifact in P6_OUTPUT_ARTIFACTS:
            path = run_dir / artifact
            self.assertTrue(path.exists(), f"Missing: {artifact}")

    def test_all_evaluations_pass_contract_validators(self) -> None:
        """All 6 evaluations bypass validate_all_evaluations."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        evals = {}
        for dim in ["market_demand", "competition", "price_profit", "voc_opportunity", "risk", "data_quality"]:
            path = run_dir / "evaluations" / f"{dim}_evaluation.json"
            evals[dim] = load_json(path)

        validate_all_evaluations(evals)

    def test_evaluation_summary_passes_contract(self) -> None:
        """evaluation_summary passes validate_evaluation_summary."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        summary = load_json(run_dir / "evaluations" / "evaluation_summary.json")
        validate_evaluation_summary(summary)

    def test_evaluation_summary_has_all_six_dimensions(self) -> None:
        """evaluation_summary.dimension_results contains all 6 dimensions."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        summary = load_json(run_dir / "evaluations" / "evaluation_summary.json")
        for dim in ["market_demand", "competition", "price_profit", "voc_opportunity", "risk", "data_quality"]:
            self.assertIn(dim, summary["dimension_results"])
    ...
    def test_evaluations_have_valid_scores_and_ratings(self) -> None:
        """Every evaluation has score 0-100 and valid rating."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)
        ...
        for dim in ["market_demand", "competition", "price_profit", "voc_opportunity", "risk", "data_quality"]:
            ...
            self.assertIn(ev["rating"], {"strong", "watch", "weak", "blocked"})
            self.assertIn(ev["confidence"], {"high", "medium", "low"})

    def test_evaluations_have_evidence_refs(self) -> None:
        """Every evaluation has non-empty evidence_refs pointing to upstream packets."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        for dim in ["market_demand", "competition", "price_profit", "voc_opportunity", "risk", "data_quality"]:
            ev = load_json(run_dir / "evaluations" / f"{dim}_evaluation.json")
            self.assertIsInstance(ev["evidence_refs"], list)
            self.assertGreater(len(ev["evidence_refs"]), 0)
    ...
    def test_summary_blocked_dimensions_tracked(self) -> None:
        """blocked_dimensions in summary matches actual blocked ratings."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        summary = load_json(run_dir / "evaluations" / "evaluation_summary.json")
        for dim in summary.get("blocked_dimensions", []):
            ev = load_json(run_dir / "evaluations" / f"{dim}_evaluation.json")
            self.assertEqual(ev["rating"], "blocked",
                           f"Summary says {dim} is blocked but rating is {ev['rating']}")

    def test_summary_low_confidence_dimensions_tracked(self) -> None:
        """low_confidence_dimensions in summary matches actual low confidence."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        summary = load_json(run_dir / "evaluations" / "evaluation_summary.json")
        for dim in summary.get("low_confidence_dimensions", []):
            ev = load_json(run_dir / "evaluations" / f"{dim}_evaluation.json")
            self.assertEqual(ev["confidence"], "low",
                           f"Summary says {dim} is low confidence but it is {ev['confidence']}")

    def test_recommended_verdict_range_is_valid(self) -> None:
        """recommended_final_verdict_range contains only valid verdicts."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        summary = load_json(run_dir / "evaluations" / "evaluation_summary.json")
        verdict_range = summary.get("recommended_final_verdict_range", [])
        self.assertGreater(len(verdict_range), 0)
        for v in verdict_range:
            self.assertIn(v, {"go", "watch", "no_go", "blocked"})

    def test_cross_dimension_tensions_present(self) -> None:
        """cross_dimension_tensions list is present (may be empty or populated)."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        summary = load_json(run_dir / "evaluations" / "evaluation_summary.json")
        self.assertIsInstance(summary.get("cross_dimension_tensions"), list)

    # ── Progress flow ─────────────────────────────────────────────────

    def test_progress_stage_8_set_to_done(self) -> None:
        """stage_8_evaluation.status = done after P6."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        progress = load_json(run_dir / "progress.json")
        stage_8 = progress["stages"].get(P6_STAGE_ID, {})
        self.assertEqual(stage_8.get("status"), "done")

    def test_progress_completed_artifacts_includes_all_outputs(self) -> None:
        """completed_artifacts accumulates all 7 P6 outputs."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)
        ...
        for artifact in P6_OUTPUT_ARTIFACTS:
            self.assertIn(artifact, completed, f"Missing from completed_artifacts: {artifact}")

    def test_progress_next_action_points_to_p7(self) -> None:
        """next_action suggests proceeding to P7."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        progress = load_json(run_dir / "progress.json")
        next_action = progress.get("next_action", {})
        self.assertIn(next_action.get("next_substage", ""), ["P7", "P6"])
    ...
    def test_p6_does_not_produce_report_or_web_artifacts(self) -> None:
        """P6 does NOT generate HTML, XLSX, or integrated_operator_judgment."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        eval_dir = run_dir / "evaluations"
        # No HTML or XLSX in evaluations/
        html_files = list(eval_dir.glob("*.html"))
        xlsx_files = list(eval_dir.glob("*.xlsx"))
        self.assertEqual(len(html_files), 0, "evaluations/ should not contain HTML")
        self.assertEqual(len(xlsx_files), 0, "evaluations/ should not contain XLSX")

        # No integrated_operator_judgment.json
        analysis_dir = run_dir / "analysis"
        if analysis_dir.exists():
            judgment_path = analysis_dir / "integrated_operator_judgment.json"
            self.assertFalse(judgment_path.exists(), "P6 should not generate integrated_operator_judgment")

    def test_p6_only_writes_evaluations_dir(self) -> None:
        """P6 only writes to evaluations/ and progress.json."""
        run_dir = self._seed_p5_3_done(35)
        run_evaluations(run_dir)

        # evaluation_summary does NOT contain a final_verdict
        summary = load_json(run_dir / "evaluations" / "evaluation_summary.json")
        self.assertNotIn("final_verdict", summary, "evaluation_summary should not have final_verdict (P7 responsibility)")
    ...
    def test_cli_smoke_help(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "build_evaluation_summary.py")],
    ...
    def test_cli_smoke_nonexistent_dir(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "build_evaluation_summary.py"), "/nonexistent/dir"],
            capture_output=True, text=True, env=_cli_env(),
        )
        self.assertNotEqual(result.returncode, 0)
    ...
    def test_cli_smoke_success(self) -> None:
        ...
        run_dir = self._seed_p5_3_done(35)
        result = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "build_evaluation_summary.py"), str(run_dir)],
            capture_output=True, text=True, env=_cli_env(),
        )
